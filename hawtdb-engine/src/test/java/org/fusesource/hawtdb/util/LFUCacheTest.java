...
public class LFUCacheTest {
    ...
    public void testElementsWithSameMaxFrequency() {
        // 根据最大值和负载因子计算淘汰策略的个数
        int max = 2;
        float evictionFactor = 0.5f;
        LFUCache<Integer, Integer> cache = new LFUCache<Integer, Integer>(max, evictionFactor);
        // 选择最近最久未使用的页面予以淘汰
        cache.put(0, 0);
        cache.put(1, 1);
        // Reach max frequency:
        ...
        cache.get(0);// 因为0这个key使用了两次，所以淘汰了key=1的这个元素
