 ...
 * Reads and writes are done at the buffers offset.
 * Every read and write increases the buffer's offset and decreases the buffer's length.
 * Bounds checking are only performed when assertions are enabled on  the JVM.
 * It's up to you to make sure there is enough data/space  in the buffer to do the read or write.
 *
 * 用于在缓冲区中写入和读取原语。读取和写入在缓冲区偏移量处完成。
 * 每次读写增加缓冲区的偏移量并减少缓冲区的长度。
 * 仅当对启用断言时才执行边界检查JVM。你要确保有足够的数据/空间 在缓冲区中执行读或写操作。
 */
